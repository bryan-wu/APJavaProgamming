...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        // find accessibility of current bestMove coordinates
        int nextAccessibility = accessGrid[bestMove.getRow()][bestMove.getCol()];

        // find the next lower accessibility to assign as bestMove
        for ( int i = 0; i < locs.size(); i++ )
        {
            if ( accessGrid[locs.get( i ).getRow()][locs.get( i ).getCol()] 
                            < nextAccessibility )
            {
                bestMove = locs.get( i );
            }
    ...
    public void updateAccess( Location loc )
    {
        ...
        for ( int i = 0; i < moves.size(); i++ )
        {
            ...
            if ( moves.contains( new Location( row, column ) ) )
            {
                accessGrid[row][column]--;
            }
        }
        accessGrid[currentRow][currentCol] = -1;
    }
