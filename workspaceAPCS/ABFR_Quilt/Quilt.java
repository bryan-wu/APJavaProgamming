...
public class Quilt
{
    ...
    public char[][] quiltToMat() // checkerboard alternation
    {
        ...
        for ( int row = 0; row < myRowsOfBlocks; row++ )
        {
            for ( int col = 0; col < myColsOfBlocks; col++ )
            {
                if ( ( row + col ) % 2 == 1 )
                {
                    placeFlipped( row * myBlock.length, col * myBlock[myBlock.length - 1].length, mat );
                }
                else
                {
                    placeBlock( row * myBlock.length, col * myBlock[myBlock.length - 1].length, mat );
                }
            }
        }
        return mat;
