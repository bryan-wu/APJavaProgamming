...
public class Ch25Exercises
{
    ...
    {
        Object numNodes = completeTree[0];
        int n = ( (Integer)numNodes + 1 ) / 2;
        completeTree[0] = (Integer)numNodes - n;
    }
     ...
     */
    private void toArray( TreeNode<Integer> node, Object[] values, int i )
    {
        if ( node == null )
        {
            return;
    ...
    public void reheapDown( Integer[] items, int numNodes )
    {
        ...
        while ( right <= numNodes + 1 && items[left] < items[index] && items[right] < items[index] )
        {
            if ( items[left] < items[right] )
            {
                Integer temp = items[index];
            ...
            if ( response.length() > 0 )
            {
                ...
                switch ( response.charAt( 0 ) )
                {
                    ...
                    case '3':
                        ...
                        System.out.println( "isHeap( items, items[0] ) = " + ex.isHeap( items, items[0] ) );
                        break;
                    case '4':
