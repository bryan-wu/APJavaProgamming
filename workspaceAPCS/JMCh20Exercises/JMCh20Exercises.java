...
public class JMCh20Exercises<E>
{
    ...
    {
        int nodeCount;
        for ( nodeCount = 0; head != null; nodeCount++ )
        {
            head = head.getNext();
        }
    ...
    public ListNode<E> reverseList( ListNode<E> head )
    {
        ...
        else
        {
            ...
            for ( ListNode<E> tempNode = head; tempNode != null; tempNode = tempNode.getNext() )
            {
                ListNode<E> newNode = new ListNode<E>( tempNode.getValue(), newHead );
                newHead = newNode;
            }
            return newHead;
        }
    ...
    public ListNode<String> concatenateStrings( ListNode<String> head )
    {
        ...
        {
            String stringSum = head.getValue();
            ListNode<String> newHead = new ListNode<String>( head.getValue(), null );
            ListNode<String> originalNewHead = newHead;
            for ( ListNode<String> tempNode = head.getNext(); tempNode != null; tempNode = tempNode.getNext() )
            {
                stringSum += tempNode.getValue();
                newHead.setNext( new ListNode<String>( stringSum, null ) );
    ...
    public ListNode<E> rotate( ListNode<E> head )
    {
        ...
        {
            ListNode<E> originalHead = head;
            ListNode<E> newHead = head.getNext();
            ListNode<E> tempNode;
            for ( tempNode = head.getNext(); tempNode != null; tempNode = tempNode.getNext() )
            {
            ...
            tempNode.setNext( originalHead );
            originalHead.setNext( null );
            return newHead;
        }
    }
    ...
    public ListNode<String> insertInOrder( ListNode<String> head, String s )
    {
        if ( head == null )
        {
            return head = new ListNode<String>( s, null );
        }
        else
        {
            ...
            {
                newNode = new ListNode<String>( s, head );
                return newNode;
            }
            else
            {

                for ( ListNode<String> tempNode = head; tempNode != null; tempNode = tempNode.getNext() )
                {
                    if ( tempNode.getValue().equals( s )
                        || tempNode.getNext() != null && tempNode.getNext().getValue().equals( s ) )
                    {
                        break;
                    }
                    if ( tempNode.getNext() != null && tempNode.getNext().getValue().compareTo( s ) > 0 )
                    {
    ...
    {
        for ( ListNode<String> node1 = head; node1 != null; node1 = node1.getNext() )
        {
            ListNode<String> minNode = node1;
            for ( ListNode<String> node2 = node1.getNext(); node2 != null; node2 = node2.getNext() )
            {
                String str = node2.getValue();
                if ( str.compareTo( minNode.getValue() ) < 0 )
                {
                    minNode = node2;
                }
            }
            String min = minNode.getValue();
            minNode.setValue( node1.getValue() );
            node1.setValue( min );
        }
    }
    ...
    {
        ListNode2<Integer> maxNode = head;
        for ( ListNode2<Integer> node = head.getNext(); node != null; node = node.getNext() )
        {
            if ( ( node.getValue() ).compareTo( maxNode.getValue() ) > 0 )
            {
                maxNode = node;
            }
        }
        if ( maxNode.getPrevious() != null && maxNode.getNext() != null )
        {
            maxNode.getPrevious().setNext( maxNode.getNext() );
            maxNode.getNext().setPrevious( maxNode.getPrevious() );
        }
        else
        {
                ...
                switch ( response.charAt( 0 ) )
                {
                    ...
                    case 'd':
                        ListNode<String> headD = buildList(
                            new String[] { "Santa", "bunny", "newt", "apple", "aardvark", "zebra" } );
