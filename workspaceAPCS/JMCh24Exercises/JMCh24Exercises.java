...
public class JMCh24Exercises
{
    ...
    // n = birthdays.length.
    /**
     * A class Date has methods int getMonth() that returns a value between // 0
     * and 11 (with 0 representing January) and int getDate() returns the // day
     * of the month represented by this Date object. The value returned // is
     * between 1 and 31 representing the day of the month that contains or //
     * begins with the instant in time represented by this Date object, // as
     * interpreted in the local time zone. Write the method
     * 
     * @param birthdays
     *            birthdays
     * @return returns true if at least three birthdays in the array fall on the
     *         same date. Your method should work in O(n) time, where n =
     *         birthdays.length.
     */
    ...
    public boolean has3OnSameDay( Date[] birthdays )
    {
        ...
        Map<Date, Integer> repetitionMap = new HashMap<Date, Integer>();
        for ( Date d : birthdays )
        {
            if ( repetitionMap.containsKey( d ) )
            {
                repetitionMap.put( d, repetitionMap.get( d ) + 1 );
            }
            else
            {
                repetitionMap.put( d, 1 );
            }
        }

        for ( int repeats : repetitionMap.values() )
        {
            if ( repeats >= 3 )
            {
                return true;
    ...
    // words.
    /**
     * A Scanner input is associated with a text file, open for reading, // that
     * contains words. Write a method
     * 
     * @param input
     *            inputs
     * @return returns the most frequently occurring word (or any one of such //
     *         words).
     */
    public String mostFrequentWord( Scanner input )
    {
        ...
        do
        {
            ...
            if ( response.length() > 0 )
            {
                ...
                switch ( response.charAt( 0 ) )
                {
                    case '1':
                        @SuppressWarnings("deprecation")
                        Date[] d = { new Date( 2010, 7, 4 ), new Date( 2010, 1, 1 ), new Date( 2010, 7, 4 ),
                            new Date( 2010, 12, 1 ), new Date( 2010, 7, 1 ), new Date( 2010, 7, 4 ) };
